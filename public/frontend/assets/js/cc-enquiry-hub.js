/* ============================================================================
   ...
   Contracts honoured (see CC-ENQUIRY-HUB-CONTRACT.md):
     - client carries routeKey only; submit POSTs JSON to /enquiries (session CSRF).
...
(function () {
  ...
  function mount(root) {
    ...
    function handleSubmit(typeObj, box, agreeBox, statusLine) {
      ...
      var submitBtn = box.querySelector('.cchub__submit');
      if (submitBtn) submitBtn.disabled = true;
      statusLine.textContent = 'Sending…';
      statusLine.setAttribute('data-status', '');

      var csrfMeta = document.querySelector('meta[name="csrf-token"]');
      var endpoint = (payload.type === 'collaborator') ? '/collaboration-requests' : '/enquiries';
      fetch(endpoint, {
        method: 'POST',
        headers: {
